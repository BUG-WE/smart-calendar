...
//显示图片
void showimage(uint16_t x, uint16_t y,char *p)
{
	uint16_t i,j,picL,picH;
	TFTLCD_SetPos(150,182,150,182);
	for(i=0;i<32*32;i++)
	{
		picL=*(p+i*2);	//??????
		picH=*(p+i*2+1);
...
//		        bColor 字符背景颜色
//================================================================================================
void Chinese_Show_one(uint16_t x, uint16_t y,  uint16_t num, uint16_t size,uint32_t fColor, uint32_t bColor)
{
	uint16_t temp,t,t1;
	uint16_t y0=y;
    ...
    for(t=0;t<csize;t++)
    {
        ...
        {
            if(temp&0x80){
            	TFTLCD_DrawPoint(x,y,fColor);
            }
            else {
            	TFTLCD_DrawPoint(x,y,bColor);
            }
            temp<<=1;
            y++;
            if((y-y0)==size)
...
#define MAX_CHAR_POSX 230
#define MAX_CHAR_POSY 390
void TFTLCD_ShowChinese(uint32_t x,uint32_t y,char *p,uint32_t fColor, uint32_t bColor)
{
	uint16_t GBK_HIGH;
	uint16_t GBK_LOW;
	uint16_t GBK_INDEX;
	...
	while(*p!='\0')
    {
    	...
    	GBK_LOW= (uint16_t)*p;
    	p++;
    	GBK_INDEX=(GBK_HIGH-129)*190+GBK_LOW-64-1;
    	Chinese_Show_one(x,y,GBK_INDEX,16,fColor,bColor);
        x+=16;
    }
...
//		        bColor 字符背景颜色
//================================================================================================
void Number_Show_one(uint16_t x, uint16_t y,  uint16_t num, uint16_t size,uint32_t fColor, uint32_t bColor)
{
	uint16_t temp,t,t1;
	uint16_t y0=y;
	uint16_t csize=(size*size/2 - 64)/8;

    for(t=0;t<csize;t++)
    {
        if(size==64){
        	temp = asc3_6432[num][t];
        }else if(size==32){
        	temp = asc4_3216[num][t];
        }else{
        	return;
        }
		for(t1=0;t1<8;t1++)
